...
def add_to_cart(request):
    if request.headers.get('X-Requseted-With')=='XMLHttpRequest':
...
def collection(request):
    ...
    return render(request,'shop/collection.html',{"c":c})

def collectionview(request,name):
    if(Catagory.objects.filter(name=name,status=0)):
        p=Product.objects.filter(category__name=name)
        return render(request,'shop/product/index.html',{"p":p,'category_name':name})
    else:
